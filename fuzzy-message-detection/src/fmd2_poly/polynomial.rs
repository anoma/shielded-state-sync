...
impl Polynomial {
    pub(crate) fn random<R: rand_core::RngCore + rand_core::CryptoRng>(degree: usize, rng: &mut R) -> Polynomial {
...
pub(crate) fn encode_coefficients(coeffs: &[Scalar], basepoint: &RistrettoPoint) -> Vec<RistrettoPoint> {
    let coeffs_encoded: Vec<RistrettoPoint> = coeffs
        .iter()
        .map(|c| *basepoint * c)
        .collect();
